...
MODEL_PATH = MODEL_DIR / "tiny_enhancer_v6_clean.pt"
...
# V6: focus on reconstruction, minimal identity constraint
LAMBDA_COMPLEX = 0.60
LAMBDA_MAG = 0.35
...
def parse_snr(path):
    ...
    if not match:
        raise ValueError(f"Cannot parse SNR: {path.name}")
...
def magnitude_l1(p, t):
    pc = torch.complex(p[:, 0], p[:, 1])
    tc = torch.complex(t[:, 0], t[:, 1])
...
def train():
    ...
    if not files:
        raise RuntimeError(f"No files in {NOISY_DIR}")
    ...
    train_loader = DataLoader(
        SpeechEnhancementDataset(train_files, True),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=0
    )
    val_loader = DataLoader(
        SpeechEnhancementDataset(val_files, False),
    ...
    print("=" * 72)
    print("PS26052 ANC - V6 CLEAN TRAINING")
    print("=" * 72)
    print(f"Key change: MINIMAL identity loss (0.05) to prevent over-suppression")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for noisy, clean in train_loader:
            ...
            loss = (
                ...
                + LAMBDA_IDENTITY * torch.mean(torch.abs(predicted - noisy))
            )
            loss.backward()
        ...
        if val_avg < best_val:
            ...
            torch.save({
                ...
                "model_name": "TinyComplexEnhancerV6",
                "sample_rate": SR, "n_fft": N_FFT,
                "hop_length": HOP_LENGTH, "win_length": WIN_LENGTH,
                "architecture": "complex_residual_v6",
                "dataset": "controlled_v3",
                "improvements": "minimal_identity_loss_focused_reconstruction",
                "best_val_loss": best_val, "epoch": best_epoch,
            }, MODEL_PATH)

    print()
    print("=" * 72)
    print(f"V6 COMPLETE | Best epoch: {best_epoch} | Val: {best_val:.6f}")
