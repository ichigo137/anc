...
def enhance_audio(input_path, output_path):
    ...
    x = torch.from_numpy(np.stack([normalized.real, normalized.imag]).astype(np.float32)).unsqueeze(0).to(DEVICE)
    ...
    waveform = librosa.istft(enhanced_complex, hop_length=HOP_LENGTH, win_length=WIN_LENGTH, length=original_length)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python inference_v6_clean.py INPUT.wav OUTPUT.wav")
        raise SystemExit(1)
