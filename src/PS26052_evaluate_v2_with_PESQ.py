...
INFERENCE_SCRIPT = ROOT / "src" / "inference_v2.py"
...
def speech_quality_pesq(clean, enhanced, sr):
    """Compute PESQ wideband score at 16 kHz."""
...
print("=" * 78)
print("PS26052 Speech Enhancement Evaluation v2 + PESQ")
print("=" * 78)
...
for i, noisy_path in enumerate(
    ...
    noisy_pesq = speech_quality_pesq(
        clean,
        noisy,
        16000
    )

    enhanced_pesq = speech_quality_pesq(
        clean,
        enhanced,
        16000
    )
    ...
    pesq_improvement = (
        ...
        if (
            np.isfinite(noisy_pesq)
            and np.isfinite(enhanced_pesq)
        )
        else np.nan
    )
...
if results:
    ...
    print(
        f"  Noisy average       : "
        f"{mean('noisy_pesq'):.3f}"
    )

    print(
        f"  Enhanced average    : "
        f"{mean('enhanced_pesq'):.3f}"
    )

    print(
        f"  Improvement         : "
        f"{mean('pesq_improvement'):+.3f}"
    )
